...
#pragma config(Motor,  port2,           drive_left,    tmotorVex393_MC29, openLoop, reversed, driveLeft, encoderPort, I2C_1)
#pragma config(Motor,  port3,           drive_right,   tmotorVex393_MC29, openLoop, driveRight, encoderPort, I2C_2)
...
int DEADBAND = 10;							//Joystick seldom 0 when off so this is the value to ignore joystick 'noise' below

//lift()
int LIFT_UP = 80;				//power to raise lift
int LIFT_DOWN = -25;		//lower lift
int LIFT_HOLD = 15;			//power to hold lift

//flip
int MOTOR_FLIP_TIME = 500;	//time to flip_motor 360 degrees
int HOLD_MOTOR_FLIP = 8;	//power to hold motor_flip
int CLAW_FLIP_TIME	= 500;	//time to flip claw 180 degrees
int HOLD_CLAW_FLIP = 8;	//power to hold claw_flip
...
task drive(){
	while(true){
		if(abs(vexRT[Ch2]) > DEADBAND){
			R_POWER = vexRT[Ch2] * DRIVE_SENSITIVITY;
		}
		else {
			R_POWER = 0;
		}
		...
		if(abs(vexRT[Ch3]) > DEADBAND){
			L_POWER = vexRT[Ch3] * DRIVE_SENSITIVITY;
		}
		else {
			L_POWER = 0;
		}
...
startTask(lift);
  ...
  while (true)
  {
  	...
  	else if(vexRT(Btn8U)==1){	//flip motor back
   		flipMotor(-1);
   	}

   	if(vexRT(Btn5U)==1){	//flip claw fwd
   		flipClaw(1, CLAW_FLIP_TIME);
   	}
  	...
  	if(vexRT(Btn8U)==1){	//open claw
   		motor[claw]=-100;
   		wait1Msec(500);
   		motor[claw]=0;
   	}
   	else if(vexRT(Btn8D)==1){	//close & hold claw
   		motor[claw]=100;
   		wait1Msec(500);
   		motor[claw]=20;
   	}
   	...
   	// !!!!!! NEED TO CONFIRM POTENTIONMETER VALUES / DIRECTION  !!!
 		if(vexRT(Btn7L)==1){	//aim arm for flag  A
   		if(SensorValue(launch_angle) < FLAG_A) //raise arm because it is lower than intended angle
			...
			if(SensorValue(launch_angle) > FLAG_A) //lower arm because it is higher than intended angle
					while(SensorValue(launch_angle) > FLAG_A){
