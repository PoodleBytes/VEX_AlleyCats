...
/*  Autonomous from Position A 	*/
	...
	while(SensorValue(Arm_Angle)<Arm_Start + 60){
		motor[L_Arm]=55;	//lift  claw
	}
...
//	tDrive(60,60,1600); //timed driving distance: Left power, Right power, Time (ms)
	eDrive(60,60,1000);		//eDrive (encoder) is roughly 62% of tDrive (timed) at 60% power
												//but is it reliable???
	/*
	//lift cap
	while(SensorValue(Arm_Angle) < Arm_Start + 75){
				motor[L_Arm]=40;}	//lift claw
	motor[L_Arm]=15;
	...
	//turn
	tDrive(-60*direction,60*direction,1800); //timed driving distance: Left power, Right power, Time (ms)
	...
	// backup
	tDrive(-100,-100,1600); // end of autoA  */
}//end autoA
